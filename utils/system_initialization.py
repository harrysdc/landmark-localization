...
def gfun(mu, u):
    '''
    motion model in equation (1)
    mu is robot state [x, y, theta] of shape (3, )
    u is noised control input [linear speed v, angular speed w, gamma] of shape (3, )
    return the new state of a robot 
    '''
    output = np.zeros(3)
...
def hfun(landmark_x, landmark_y, mu_pred):
    '''
    Measurement Model in equation (7)
    '''
    output = np.zeros(2)
